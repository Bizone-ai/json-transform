...
import { convertFunctionsToObjects, tryConvertFunctionsToInline } from "../../../functions/utils/convert";
...
describe("convert", () => {
  ...
  test("object to inline - test 10 ($$raw)", () => {
    expect(
      tryConvertFunctionsToInline({
        $$raw: {
          $$math: [1, "/", 2],
  ...
  test("object to inline - test 11 ($$raw)", () => {
    expect(
      tryConvertFunctionsToInline({
        $$raw: "$$pad(LEFT,8):$.total",
      }),
    ).toEqual("$$raw:$$pad(LEFT,8):$.total");
  });
