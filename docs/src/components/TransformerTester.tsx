...
const TransformerTester = () => {
  ...
  const [outputIsString, setOutputIsString] = useState(false);
  ...
  useEffect(() => {
    if (parsedInput) {
      ...
      setSuggestions(
        "/transformer",
        Object.keys(ContextVariablesSchemas).concat(result?.paths.map(x => jsonpathJoin("$", x.$path)) ?? []),
    ...
    <>
      <div className="tester_grid">
        ...
        <h3 style={{ display: "flex", flexDirection: "row-reverse", justifyContent: "space-between" }}>
          ...
          <span className="hide-on-mobile">Output {outputIsString ? "(string)" : ""}</span>
        </h3>
        <MonacoEditor
          language="json"
          value={inputString}
        ...
        />
        <MonacoEditor
          language={outputIsString ? "text" : "json"}
          readOnly
        ...
        <div className="button-container">
          ...
          <button
            type="button"
            className="button button--primary transform-button shadow--lw"
        ...
        <div
          style={
            outputError ? { color: "red", fontSize: "0.7em" } : outputJSONString ? { fontSize: "0.7em" } : undefined
          }
        >
          {outputError ??
            (outputJSONString
      ...
      {modalOpen && (
        <Modal title="Visualize Transformer" onRequestClose={() => setModalOpen(false)}>
          <MarkmapView
            style={
              {
